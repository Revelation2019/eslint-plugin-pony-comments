...
/**
 * @description 获取当前节点的前一个节点
 * @param {*} context
 * @param {*} node
 * @param {*} countOptions
 * @returns
 */
const getTokensBefore = (context, node) => {
  return context.getSourceCode().getTokensBefore(node, { count: 1 });
};
...
/**
 * @description 生成jsx中引入组件的注释
 * @param {*} node
 * @param {*} text
 * @returns
 */
const genJsxHeadComments = (node, text) => {
  if (!text) return null;
  const eol = require('os').EOL;
  const content = `{/** ${text} */}${eol}`;
  return (fixer) => {
    return fixer.insertTextBefore(
      node,
      content,
...
/**
 * @description 判断JSX组件的注释
 * @param {*} comments
 * @param {*} node
 * @returns
 */
const judgeJSXExpressionContainerComments = (comments, node) => {
  return comments.some(c => c.range.sort().toString() === node.expression.range.sort().toString());
};
