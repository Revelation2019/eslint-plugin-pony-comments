...
/**
 * @description 返回给定节点的第一个token
 * @param {*} context
 * @param {*} node
 * @param {*} countOptions
 * @returns
 */
const getNodePrevTokens = (context, node, countOptions) => {
  return context.getSourceCode().getTokensBefore(node, countOptions);
};
...
const judgeNodeType = (node, type) => {
  ...
  if (Array.isArray(type)) {
    return type.some(t => node.parent.type === t);
  }
  return false;
};
...
/** 给节点添加小括号 */
const genBracketsAndHeadComments = (node, text) => {
  const eol = require('os').EOL;
  return (fixer) => {
    return [fixer.insertTextBefore(
      node,
      ...
      node,
      `)${eol}`,
    ), genHeadComments(node, text, 'block')(fixer)];
  };
};

/**
 * @description 自定义组件头部加注释
 * @param {Object} node 当前节点
 * @param {String} text 注释内容
 */
const genCustomComponentHeadComments = (node, text, isEol = true) => {
  if (!text) return null;
  const eol = require('os').EOL;
  const content = `${isEol ? `{/** ${text} */}${eol}` : `{/** ${text} */}`}`;
  return (fixer) => {
    return fixer.insertTextBefore(
      node,
      content,
