          ...
          if (init && id) {
            ...
            if (type === 'ArrowFunctionExpression') {
              if (judgeNodeType(node, 'ExportNamedDeclaration')) {
                ...
                if (!leading.length || getLastEle(leading).loc.start.column !== parent.loc.start.column) {
                  context.report({
                    node: parent,
                    message: '导出的箭头函数头部没有注释',
                    fix: genHeadComments(parent, name, commentsType),
                  });
                }
              } else {
                /** const aaa = 111; */
                ...
                const { leading } = getComments(context, node);
                /** 节点前注释开头列位不等于节点开头列位，意味着节点前没有注释 */
                if (!leading.length || getLastEle(leading).loc.start.column !== node.loc.start.column) {
                  context.report({
                    node: node,
                    message: '箭头函数头部没有注释',
                    fix: genHeadComments(node, name, commentsType),
