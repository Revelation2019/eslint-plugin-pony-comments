    ...
    return {
      'JSXElement': (node) => {
        ...
        if (!baseElement.includes(n) && !bricks.includes(n)) {
          ...
          if (preNodeOfJSXElement.type === 'JSXText') {
            const preNodeOfJSXText = getTokensBefore(context, node);
            if (preNodeOfJSXText.type !== 'JSXExpressionContainer' || preNodeOfJSXText.expression.type !== 'JSXEmptyExpression' || !judgeJSXExpressionContainerComments(comments, preNodeOfJSXText)) {
              report(
                context,
                node,
                '自定义组件或者业务组件没有写注释',
          ...
          } else if (preNodeOfJSXElement.type === 'JSXExpressionContainer'){
            if (preNodeOfJSXElement.expression.type !== 'JSXEmptyExpression' || !judgeJSXExpressionContainerComments(comments, preNodeOfJSXElement)) {
              report(
                context,
                node,
                '自定义组件或者业务组件没有写注释',
