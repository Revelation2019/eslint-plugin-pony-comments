        ...
        if (judgeNodeType(node, 'TSEnumDeclaration')) {
          ...
          if (commentsType === 'Block' || (commentsType === 'Line' && pos === 'lead')) {
            ...
            report(context, node, errorMsg, (fixer) => {
              const startRange = getLastEle(node.range) + 2;
              return fixer.insertTextAfterRange({
                range: [startRange, startRange + `// ${content}`.length],
                text: `// ${content}`,
              });
            });
          }
          ...
          //   report(context, node, '枚举定义的字段没有注释', genHeadComments(node, content, commentsType));
          // }
        }
      },
    };
